...
def get_session_manager(config):
    if "session_db_path" in config:
        logging.info("Found session_db_path in config, using SqliteSessionManager for auth")
        return SqliteSessionManager(config['session_db_path'])
    elif "session_manager" in config:  # load from config
        logging.info(f"Found session_manager in config, using {config['session_manager']} for persisting sessions")
        import importlib
        import inspect
        ...
        logging.warning("Neither session_db_path nor session_manager set, "
