...
def get_user_manager(config):
    if "auth_db_path" in config:
        logging.info("Found auth_db_path in config, using SqliteUserManager for auth")
        return SqliteUserManager(config['auth_db_path'], config['data_root'])
    elif "user_manager" in config:  # load from config
        logging.info(f"Found user_manager in config, using {config['user_manager']} for auth")
        import importlib
        import inspect
        ...
        logging.warning("neither auth_db_path nor user_manager set, ankisyncd will accept any password")
